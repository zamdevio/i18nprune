# `runReview` SDK Example

Programmatic use of `@i18nprune/core`'s `runReview` from a Node script. This is the same locale review aggregation the CLI command uses; the CLI adds context resolution, cached report loading, JSON envelope shaping, and human summaries.
...
4. Implementing `ReviewHostHooks` for dynamic-site counts.
5. Calling `runReview(ctx, opts, host)`.
6. Reading `payload` and `issues`.

## Files

- `runReview.ts` — the example, top-to-bottom commented.
- `i18nprune.config.ts` — project config authored with `defineConfig`.
- `locales/en.json` — sample source locale.
- `locales/fr.json` — sample target locale with structured and legacy leaves.
...
- **Core owns locale reads and aggregation.** It reads source/target JSON via adapters and applies parity exclusions from config.
