...
2. Loading config + source locale through host fs.
...
- `i18nprune.config.json` — project config (`mymemory` provider, no API key).
...
`mymemory` is a public translator with rate limits — fine for the small fixture here, but expect failures if you scale up.
