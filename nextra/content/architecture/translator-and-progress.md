...
|-------|------|------------|
| **Provider** | HTTP/API: `translate(text, sourceLang, targetLang) → Promise<string>` | `src/providers/google/`, `createTranslator()` in `src/core/translator/init.ts` |
| **Leaf pipeline** | Placeholders: mask → translate → restore → validate; **3 retries** with backoff | `src/core/translator/index.ts` (`translateLeaf`) |
| **Types** | `Translator`, `TranslateRequest` (request shape for future use) | `src/types/core/translator/index.ts` |
| **Progress** | TTY **stderr** multi-line or single-line `tick` / `done` / `fail`; **no-op** when JSON / quiet / silent | `src/core/progress/index.ts`, `translation.ts`, `session.ts` |
| **Session** | SIGINT → exit **130**, stdin discard during progress | `src/core/progress/session.ts` (`createSessionProgress`) |
| **Logger policy** | `canPrintProgress`, `canPrintInfo`, … | `src/utils/logger/policy.ts` |
...
3. **Optional `@types`** — Keep **`src/types/core/translator`** as the single contract; add **`TranslatorResult`** / **`TranslatorFailure`** only if we need discriminated unions for **`--json`** summaries.
