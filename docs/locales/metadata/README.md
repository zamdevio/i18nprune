...
Shared writer behavior for `sync`, **`generate`**, and **`generate --resume`**.
...
- **`generate --metadata`** (CLI): the CLI tells core to **persist** structured leaves built from **`translateLeaf`** output (`value` + merged **`TranslationLeafMeta`**, including provider/heuristic scores and **`needsTranslationAgain`**). Core **always** computes that metadata; the flag only gates **writing** to disk.
...
- `sync --metadata` / `generate --metadata`
...
This report is attached in per-command JSON payload rows (`targetResults` for **`generate`**, and `localeMetadataReports` for `sync`) and can also be used directly in API integrations.
