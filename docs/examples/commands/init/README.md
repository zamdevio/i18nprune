...
Generated **`i18nprune.config.ts`** uses **`import { defineConfig, type I18nPruneConfig } from 'i18nprune/core/config'`** and **`satisfies Partial<I18nPruneConfig>`** so your editor type-checks the object against the CLI schema (minimal and **`--rich`** templates both include this).
