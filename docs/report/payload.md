...
| Field | Value |
|-------|--------|
| `kind` | Must be exactly `i18nprune.projectReport` (`PROJECT_REPORT_KIND` in `packages/report`, workspace `@i18nprune/report-schema`). |
...
The authoritative schema is **`projectReportDocumentSchema`** (Zod) in **`packages/report`** (workspace **`@i18nprune/report-schema`**). Published installs import the same types via the root **`i18nprune/report`** export.
...
- [Report UI (embedded SPA)](./README.md)
