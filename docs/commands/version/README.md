...
- **`version --check`** — fetches **`@zamdevio/i18nprune`** **`latest`** from the **npm registry**, prints current vs published, and suggests **`npm i -g @zamdevio/i18nprune`** when an upgrade exists. Respects **`I18NPRUNE_NO_UPDATE_CHECK`** (see [Environment variables](../../config/env.md) and [Versioning](../../versioning/README.md)).
