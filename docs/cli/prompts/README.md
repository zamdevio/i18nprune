...
For **Inquirer scope, `--json`, CI, and utils debt**, see the canonical page: **[Prompts & CLI boundaries](../../prompts/README.md)**.
...
- **No prompts** when **`--json`** is the contract for machine-readable stdout (see [JSON output (`--json`)](../../json/README.md)).
...
- [JSON output (`--json`)](../../json/README.md) — when `--json` is set, stdout is the machine-readable contract (no interactive prompts).
...
- [Command orchestration boundary](../../commands/orchestration/README.md) — file layout inside `commands/<name>/`.
