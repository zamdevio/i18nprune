...
4. **`--json` means “no surprises on stdin”** — global **`--json`** forces non-interactive duplicate-config resolution; implicit config bootstrap matches **`--yes`** when no config exists; commands that emit JSON should prefer **`issues[]`** in the stdout envelope over stderr-only errors when work cannot complete (see [Issue codes](../issues/README.md)).
...
- [Issue codes (`issues[]`)](../issues/README.md)
