...
3. **Config resolution** — duplicate `i18nprune.config.*` selection and **`init`** format prompts live in `packages/cli/src/config/**` with **`shouldSkipInteractivePrompts()`** and global **`--json`** / **`--yes`** handling (see [Behavior: interactivity](../behavior/README.md)).
4. **`--json` means “no surprises on stdin”** — global **`--json`** forces non-interactive duplicate-config resolution; implicit config bootstrap matches **`--yes`** when no config exists; commands that emit JSON should prefer **`issues[]`** in the stdout envelope over stderr-only errors when work cannot complete (see [Issue codes](../json/issue-codes.md)).
...
## Report / `--report-file` path collisions
...
- [Behavior: exit codes and interactivity](../behavior/README.md)
- [Config](../config/README.md)
- [Issue codes (`issues[]`)](../json/issue-codes.md)
- [JSON output](../json/README.md)
