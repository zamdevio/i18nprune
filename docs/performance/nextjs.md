...
That produces **`i18nprune.config.ts`** using **`defineConfig`** / **`I18nPruneConfig`** from **`i18nprune/core/config`** (see [Configuration](../config/README.md)). Tune **`source`**, **`localesDir`**, **`src`**, **`functions`**, **`exclude`** for your tree.
...
| **`--exclude tests,bench`** (comma-separated **directory basenames**) | Extra skips for **this** invocation ([precedence](../config/exclude.md), [CLI overview](../cli/README.md)). |

**Impact:** skipping tests, benches, compiled bundles, and generated trees routinely drops scanned file counts and often **cuts wall time and RSS** by a multiple compared to scanning the whole checkout ([Performance hub](./README.md#exclude-shrink-what-gets-scanned)).
...
- Dynamic detection is **permissive by design** ([Dynamic keys](../dynamic/README.md)).

### What to do in practice

- Narrow **`src`**, **`exclude`**, and **`--exclude`** ([Performance hub](./README.md#exclude-shrink-what-gets-scanned), [Exclude](../config/exclude.md)).

---

## Report UI

HTML output opens locally; links use IDE schemes where applicable ([Report UI](../report/README.md)).

## Conclusion

- Counts assume **`src: '.'`** (~**20k** files scanned) unless you tighten **[exclude](./README.md#exclude-shrink-what-gets-scanned)**.
