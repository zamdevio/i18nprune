...
| Axis | Meaning |
|------|--------|
| **Scan scale** | Number of source-like files under `src` (see [config `src`](../config/README.md)) that participate in key-site and dynamic scans. |
...
| **Correctness** | Paths and spans in reports match files on disk; extraction rules are documented under [regex](../regex/README.md) and [dynamic keys](../dynamic/README.md). |

## What is measured

- **File count** — files matching scanable extensions under the resolved `src` root (same pipeline as [`validate`](../commands/validate/README.md) / [`report`](../commands/report/README.md)).
...
- **Extractor outputs** — counts of literal observations, dynamic sites, and missing keys relative to the source locale JSON (semantics in [JSON output](../json/README.md) and command docs).
...
- Global **`--exclude`** — comma-separated directory **basenames** merged into the skip list ([CLI overview](../cli/README.md)).
...
| [CepatEdge — CLI workflow](./cepatedge.md) | [CepatEdge](https://github.com/zamdevio/cepatedge) (planned / linked from [Origin](../origin/README.md#open-source-plan-for-cepatedge)) | **Which** commands to run, **when** to time runs, tiered checklist (3 / 5 / 8 commands). |
