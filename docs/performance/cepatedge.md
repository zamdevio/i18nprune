...
Snapshot from **`report --format json`** on the **pinned** config path / product revision in the table above — your tree may differ; **[When to refresh benchmark pages](./README.md#when-to-refresh-benchmark-pages)**.
...
On this smaller tree **warm vs cold** is modest compared to **[large-repo cache effects](./README.md#cli-project-cache)**.

### Exclude & cache pointers

[**Exclude**](../config/exclude.md) and [**CLI project cache**](../cli/cache.md) matter most when **`src`** is wide ([Performance hub](./README.md)). This **`apps/web`** config uses **`exclude.preset: 'production'`**.
...
- **`functions`** — Match i18next / wrappers ([config](../config/README.md), [dynamic](../dynamic/README.md)).
...
| Report | [Report command](../commands/report/README.md) |
| Exclude | [Exclude](../config/exclude.md) |
| CLI cache | [CLI cache](../cli/cache.md) |
| Global flags | [CLI overview](../cli/README.md) |
