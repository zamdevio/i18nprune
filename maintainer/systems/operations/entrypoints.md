...
| **share** | core: `packages/core/src/share/run.ts` → **`runShare`** · `list.ts` / `view.ts` / `delete.ts` · **`buildProjectPayload`** / **`buildReportPayload`** · `human.ts` → **`emitShare*HumanMessages`** · `resolveShareWorkerBaseUrl`** · `share/io/shareJson.ts` · `remote.ts` · `run.share.*` events. CLI: `packages/cli/src/commands/share/` (`workerHttp.ts`, `workerUrl.ts`). Worker: `apps/workers/i18nprune/src/routes/v1/`. Types: `packages/core/src/types/share/*` | disk cache + worker DELETE default on `share delete`; hosts pass resolved worker URL |
