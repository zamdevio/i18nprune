...
| **share** | core: `packages/core/src/share/run.ts` → **`runShare`** · **`buildProjectPayload`** · `loadShareJsonFile` / `saveShareJsonFile` · `parseWorkerShareEnvelope` / `shareRemoteIssueFromWorker` · `run.share.*` events ([`apps.md`](../../phases/apps.md) tracker) | disk cache + zip manifest + worker response mapping; CLI/web hosts |
