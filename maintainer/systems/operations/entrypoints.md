...
| **share** | `packages/core/src/share/` — **`loadShareJsonFile`**, **`saveShareJsonFile`**, **`parseWorkerShareEnvelope`**, **`shareRemoteIssueFromWorker`** (orchestrator **`runShare`** in [`apps.md`](../../phases/apps.md) tracker row 1+) | disk cache + worker response mapping; CLI/web hosts |
