...
Root config applies to the whole repo; nested keys under **`workspaces`** tune **`apps/landing`**, **`apps/extension`**, and **`apps/extension/src/webview`** (separate Vite app with its own `package.json`).
