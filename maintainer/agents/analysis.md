...
## 2. Repository layout (high level, planned core split)
...
| `docs/` | **VitePress-source markdown** — One `README.md` per topic. |
...
**Git hygiene:** Maintainer phase ordering lives in **`maintainer/phases/README.md`** (outside **`docs/`** — **not** copied to the docs site). Public direction stays in **`docs/roadmap/README.md`**.
...
- **Schema:** Zod-validated (`packages/cli/src/config/schema.ts`) — required **`source`**, **`localesDir`**, **`src`**, **`functions`**; optional **`noLocaleMeta`**, **`policies`**, **`reportFormat`**, **`exclude`**, **`scanner`**, **`reference`**, **`localeLeaves`**, **`missing`**, **`patching`**, **`translate`**, etc. (**no** **`config.validate`** namespace today — **`validate`** follows CLI-only flags).
...
- **Integration:** `tests/integration/cli.fixture.test.ts` runs **`dist/cli.js`** against the sample fixture app — requires **`pnpm run build`** first.
- **Policy:** Run **`pnpm typecheck`** and **`pnpm test`** before large merges; agents should do the same (see `maintainer/agents/rules.md`).
...
- **Per-command docs:** `docs/commands/<command>/README.md` — should stay in lockstep with `packages/cli/bin/cli.ts` and `packages/cli/src/commands/` (see **`maintainer/agents/git.md`** for the commit plan).
- **Behavior:** `docs/behavior/` — exit codes, JSON/long-run interaction.
- **Architecture:** `docs/architecture/` — ADRs, translator/progress, tree.

---

## 9. Extension points (planned)
...
## 11. First tasks for a new agent (planned core architecture)

1. Read **`maintainer/agents/rules.md`** and **`maintainer/agents/git.md`**.  
...
5. For feature work, follow the **commit plan** in **`maintainer/agents/git.md`** so history stays reviewable.
