...
| **Share (C.3+)** — core `share.json` load/save + self-heal + worker envelope mapping + **`runShare`** project/report payload validation + `runShareList`/`runShareView`/`runShareDelete` + `run.share.*` events | Shipped (tracker rows 0–3) | [`apps.md`](./apps.md) |
