...
| **Share (C.3+)** — core `share.json` + **`runShare`** + list/view/delete + human emit + **`core/project/parseZip`** + worker CRUD + worker `routes/v1/` + CLI **`i18nprune share`** (`upload` / `list` / `view` / `delete --all`) | Shipped (tracker rows 0–6, 6b) | [`apps.md`](./apps.md) · [`docs/commands/share`](../../docs/commands/share/README.md) |
| **Share hardening (6b)** — `share.bak/` raw backups, cache-epoch skip, view 404 purge, `--debug-cache`, `cache_empty` / `stale_cache_row_removed` issues, core layout `ops/` + `cache/` + `payload/` | Shipped (core + CLI) | [`apps.md`](./apps.md) § 1.4b · [`docs/issues/share.md`](../../docs/issues/share.md) |
| **Worker hardening (6c)** — prepared + archive ingest, structured worker API errors, IP rate limit, `HASH_ALREADY_EXISTS` + `?force=true`, storage-pressure eviction (~25% project/report rows), snapshot timings (`preparedAt`, `requestReceivedAt`, `storedAt`, `edge.persistMs`), `localeTags` + archive `filesEpoch`, TS/`.mjs` config `mode`/`structure` parse | Shipped (core + worker + web read compat) | [`apps.md`](./apps.md) § 3.4 · [`docs/commands/share`](../../docs/commands/share/README.md) · [`docs/runtime/worker.md`](../../docs/runtime/worker.md) |
| **Web share UX (7)** — `/#/workspace?id=` open/hydrate, link-only Share when remote, `shareRemoteIssueFromWorker` banners, prepared JSON upload default + optional archive, local prepare via `prepareProjectSnapshotFromArchive` | Shipped (`apps/web`) | [`apps.md`](./apps.md) § 4 · [`docs/commands/share`](../../docs/commands/share/README.md) |
| **Report share UX (8)** — `/#/?id=` deep link, `GET /v1/reports/:id` + `/document` hydrate, worker URL setting, open-shared-link + share upload / link-only, `buildReportShareUrl` | Shipped (`apps/report`) | [`apps.md`](./apps.md) · [`docs/commands/share`](../../docs/commands/share/README.md) |

**Next vertical:** **[`apps.md`](./apps.md)** — row **9** (`runReport`), **W** (metadata). Hub: **[`V1-RELEASE.md`](./V1-RELEASE.md)**.
