...
**Execution order (v1, after shipped Session C):** **init (F)** → **locales (H)** → **apps catch-up (C.3)** (`apps/web`, `apps/workers/i18nprune`) → **docs (D)** → **landing (D.2)** → **release (E)** + **`final.md` (G)**. Hosted apps are **deferred** until init/locales stabilize **`@i18nprune/core`** contracts — see **[`phases/V1-RELEASE.md` § Recommended sequence](phases/V1-RELEASE.md#recommended-v1-sequence-start-here-after-shipped-session-c)**.
