...
function buildRichInitConfigTemplate(importSpecifier: string): string {
  return `import { defineConfig, type I18nPruneConfig } from '${importSpecifier}';
...
export function buildInitConfigTemplate(
  importSpecifierOrOpts?: string | BuildInitConfigTemplateOptions,
): string {
  if (typeof importSpecifierOrOpts === 'string') {
    return buildMinimalInitConfigTemplate(importSpecifierOrOpts);
  }
  const opts = importSpecifierOrOpts ?? {};
  const importSpecifier = opts.importSpecifier ?? DEFAULT_INIT_CONFIG_IMPORT_SPECIFIER;
  return opts.rich ? buildRichInitConfigTemplate(importSpecifier) : buildMinimalInitConfigTemplate(importSpecifier);
}
