...
import { clearContextCache, resolveContext } from '@/core/context/index.js';
import { resetConfigPathResolution, setConfigPath } from '@/config/resolve/scan.js';
import { computeMissingLiteralKeys } from '@/core/validate/missingLiterals.js';
...
describe('computeMissingLiteralKeys', () => {
  ...
  it('returns dotted paths present in code but not in locale JSON', () => {
    const ctx = resolveContext(dir);
   ...
   */
  it('does not report false missing when two files reuse the same const name (NS)', () => {
