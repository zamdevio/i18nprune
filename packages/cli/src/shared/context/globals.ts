...
export function resetCliGlobals(): void {
  overrides = {};
  argvJsonFlag = false;
  listTopFromArgv = undefined;
  listFullFromArgv = false;
}
...
/** Global list-window CLI overrides (`--top` / `--full`). */
let listTopFromArgv: number | undefined;
let listFullFromArgv = false;

export function setCliListTopFlag(value: number | undefined): void {
  listTopFromArgv = value;
}

export function getCliListTopFlag(): number | undefined {
  return listTopFromArgv;
}

export function setCliListFullFlag(value: boolean): void {
  listFullFromArgv = value;
}

export function getCliListFullFlag(): boolean {
  return listFullFromArgv;
}
