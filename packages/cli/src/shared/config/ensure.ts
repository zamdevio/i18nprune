...
function displayConfigPathForCwd(cwd: string, absPath: string): string {
...
export async function ensureConfigPathResolved(cwd = process.cwd()): Promise<void> {
  ...
  if (explicit) {
    ...
    if (!existsRuntimeFsSync(abs, fs)) {
      setChosenImplicitPath(null);
      formatExplicitConfigError(cwd, abs, 'missing');
      process.exit(1);
      return;
    }
    if (await Promise.resolve(fs.statKind(abs)) !== 'file') {
      setChosenImplicitPath(null);
    ...
    if (others.length > 0) {
      logger.warn(
        `Multiple ${CLI_NAME} config files in this directory; using ${displayConfigPathForCwd(cwd, abs)} from --config. Ignored: ${others.map((p) => displayConfigPathForCwd(cwd, p)).join(', ')}`,
      );
    }
  ...
  const chosen = await duringPrompt(() =>
    select({
