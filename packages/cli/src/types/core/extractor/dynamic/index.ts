/**
 * Dynamic (non-literal) translation call sites — shared by validate, `locales dynamic`, and `@/core/extractor/dynamic`.
...
export type DynamicKeySite = {
   ...
   * `${…}` that cannot be resolved from the const map. Helps downstream tooling without
   * pretending the full key is known.
   */
