...
export async function ensureConfig(opts: EnsureConfigOptions = {}): Promise<void> {
  ...
  /** Same as --yes / CI: no format picker; optional default file write. Global --json counts as non-interactive here so `validate --json` never blocks on Inquirer. */
  const nonInteractiveInit =
    shouldSkipInteractivePrompts() || opts.yes || getArgvJsonFlag();
