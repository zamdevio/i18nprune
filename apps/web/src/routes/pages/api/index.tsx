...
const CONFIG_FULL = `import { defineConfig } from '@zamdevio/i18nprune/config';
...
const dynamic = scanProjectDynamicKeySites(ctx);`

const CORE_NAMESPACED = `import { context, extractor, dynamic, validate, files } from '@zamdevio/i18nprune/core';
...
export default function ApiPage() {
  useRevealOnPage("api");
  const docsJsonReportVsCliReadmeUrl = `${LINKS.docs}/json/README#${DOCS_JSON_REPORT_VS_CLI_ANCHOR}`;
  ...
  return (
    <main className="bg-background pb-24 md:pb-32">
      <PageHero
        ...
        description={
          <>
            Two supported surfaces: the binary with global{" "}
            <code className="font-mono text-[0.85em]">--json</code> (CI, pipes, files) and{" "}
            <code className="font-mono text-[0.85em]">@zamdevio/i18nprune/core</code>{" "}
            <code className="font-mono text-[0.85em]">run*</code> helpers that emit the same{" "}
            <code className="font-mono text-[0.85em]">CliJsonEnvelope</code>. The default package export is still the
            CLI — do not import it as a library. Typed config authoring lives under{" "}
            <code className="font-mono text-[0.85em]">/config</code>.
          </>
        }
      >
        <a
      ...
      <div className="mx-auto max-w-6xl space-y-10 px-4 pt-8 sm:space-y-14 sm:px-6 sm:pt-10">
        <aside className="reveal rounded-lg border border-sidebar-primary/25 bg-sidebar-primary/5 px-4 py-3 text-left text-sm leading-relaxed text-muted-foreground md:text-base">
        ...
        <aside className="reveal rounded-lg border border-border/80 bg-secondary/20 px-4 py-3 text-left text-sm leading-relaxed text-muted-foreground md:text-base">
          <strong className="font-medium text-foreground">Config and CI:</strong> global{" "}
          <code className="font-mono text-xs">--json</code>, duplicate config files, and missing config interact with
          prompts and exit codes as documented in{" "}
          <a
            href={docsUrl("behavior/README")}
            target="_blank"
            rel="noopener noreferrer"
            className="font-medium text-sidebar-primary underline-offset-4 hover:underline"
          >
          ...
          (same summary as on the <Link to="/commands" className="font-medium text-sidebar-primary underline-offset-4 hover:underline">commands</Link> page).
        </aside>
        <section className="reveal space-y-6 rounded-xl border border-border bg-card p-5 sm:p-6 md:p-8">
          <SectionIntro title="CLI `--json` and automation">
            ...
            <p>
              ...
              <DocLink href={docsUrl("json/README")}>JSON output (`--json`)</DocLink>,{" "}
              <DocLink href={docsUrl("behavior/json-long")}>long-running commands + `--json`</DocLink>,{" "}
              <DocLink href={docsUrl("json/issue-codes")}>issue codes</DocLink>.
            </p>
          </SectionIntro>
        ...
        <section className="reveal reveal-delay-1 space-y-6 rounded-xl border border-border bg-card p-5 sm:p-6 md:p-8">
          <SectionIntro title="Same envelope from TypeScript">
            ...
            <p>
              <strong className="font-medium text-foreground">Go deeper:</strong>{" "}
              <DocLink href={docsUrl("json/programmatic")}>Programmatic API + CLI JSON contract</DocLink>,{" "}
              <DocLink href={docsUrl("exports/core")}>Core exports</DocLink>.
            </p>
          </SectionIntro>
        ...
        <section className="reveal space-y-6 rounded-xl border border-border bg-card p-5 sm:p-6 md:p-8">
          <SectionIntro title="@zamdevio/i18nprune/config">
            <p>
              <strong className="font-medium text-foreground">What:</strong>{" "}
              <code className="font-mono text-xs">defineConfig</code> narrows to{" "}
              <code className="font-mono text-xs">I18nPruneConfig</code>; no filesystem I/O until{" "}
              <code className="font-mono text-xs">core</code> or the CLI resolves it.
            </p>
            <p>
              <strong className="font-medium text-foreground">Go deeper:</strong>{" "}
              <DocLink href={docsUrl("exports/config")}>Configuration API</DocLink>,{" "}
              <DocLink href={docsUrl("commands/config/README")}>config command</DocLink>.
            </p>
          </SectionIntro>
        ...
        <section className="reveal reveal-delay-1 space-y-6 rounded-xl border border-border bg-card p-5 sm:p-6 md:p-8">
          <SectionIntro title="@zamdevio/i18nprune/core — scanning without the envelope">
            <p>
            ...
            <p>
              <strong className="font-medium text-foreground">Go deeper:</strong>{" "}
              <DocLink href={docsUrl("exports/examples")}>Examples & recipes</DocLink>,{" "}
        ...
        <section className="reveal reveal-delay-2 space-y-6 rounded-xl border border-border bg-card p-5 sm:p-6 md:p-8">
          <SectionIntro title="Flat imports vs namespaced core">
            ...
            <p>
              <strong className="font-medium text-foreground">Go deeper:</strong>{" "}
              <DocLink href={docsUrl("exports/core")}>Core API — flat vs namespaced</DocLink>.
            </p>
          </SectionIntro>
        ...
        <section className="reveal rounded-xl border border-border bg-secondary/30 p-5 sm:p-6 md:p-8">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Install: <DocLink href={LINKS.npm}>npm</DocLink> · <DocLink href={docsUrl("workflow/README")}>Workflow</DocLink>
          </p>
        </section>
