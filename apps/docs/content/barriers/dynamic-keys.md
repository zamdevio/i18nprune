...
- **Ambiguity is for config files**, not dynamic keys: multiple `i18nprune.config.*` files without `--config` will **exit 1** in non-interactive environments (see [config docs](../config)).
