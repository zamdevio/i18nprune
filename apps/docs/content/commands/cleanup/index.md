...
**Full examples:** [cleanup examples](../../examples/commands/cleanup)
...
Static key usage is inferred with **regex/heuristics**, not a full typechecker. Indirect patterns (e.g. aliasing `t`) may not be seen. Read [Detection limits](../../regex#detection-limits) and [extraction](../../regex/extraction.md) before trusting aggressive cleanup.
...
With **global** **`stdout redirection`** / **`--format`**, supported runs append a structured artifact (see [report](../report)).
...
- [Roadmap](../../roadmap) — reporting / `stdout redirection` sequencing
