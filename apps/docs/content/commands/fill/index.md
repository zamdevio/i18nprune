...
**Full examples:** [fill examples](../../examples/commands/fill)
...
- **Global `--json`** — Emits one primary **`CliJsonEnvelope`** on stdout (same contract as other JSON commands). Use **`--target`** or **`--all`** when non-interactive; see [JSON output](../../json) and **`runFill`** in **`i18nprune/core`**.
...
- Uses the same **`createSessionProgress`** pipeline as **`generate`** (rich stderr progress when **`canPrintProgress`** allows; stdin restored after the run). Policy: [Translation progress](../../progress).
- Translation uses **`translateLeaf`** (shared with **`generate`**). See [Translator & progress](../../translator).
...
Repeated **unchanged** translations trigger the same **warning / abort** guard as **`generate`**. **Human** mode can confirm; **global `--yes`** continues without prompts in non-interactive runs. **`--json`**: abort yields **`ok: false`** and **`issues[]`** (`i18nprune.translate.identity_streak_*`). See [issue codes](../../issues).
...
- [Policies](../../config/policies) — `policies.parity`
- [Locales metadata mode](../../locales/metadata)
