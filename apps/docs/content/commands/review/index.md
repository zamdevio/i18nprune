...
**Full examples:** [review examples](../../examples/commands/review)

**Summary-only** locale audit vs the **source** JSON: how many string paths exist, how many still **match the source** (English-identical / parity hint), and—when leaves use **structured objects**—aggregates for **`needsReview`**, **confidence**, **status**, and **source**.
...
A future **opt-in migration** (or generate/fill mode) to emit structured locale JSON would be a separate, explicitly versioned feature—watch **[Roadmap](../../roadmap)** / **[Patching](../../patching)** if it lands, not something `sync` does silently today.

## Related

- [JSON mode and long commands](../../behavior/json-long.md)
- [Validate](../validate) · [Quality](../quality) · [Report](../report)
