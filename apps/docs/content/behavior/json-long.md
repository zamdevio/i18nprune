...
This page describes **how machine-readable mode and long-running work interact** with **prompts**, **progress**, and **stdout**. Release sequencing and **`stdout redirection`** are tracked in the **[Roadmap](../roadmap)** and maintainer **`maintainer/phases/README.md`**.

## Two different meanings of “JSON mode”

1. **Global argv `--json`** — recorded for duplicate-config / non-interactive behaviour (see [CLI overview](../cli)).
...
That way **`generate --json`** and **`fill --json`** are predictable in scripts and CI. For **`generate`**, non-interactive defaults for catalog-backed fields (when **`--lang`** is valid) are defined in [commands/generate](../commands/generate) and [Exit codes & behavior](.).
...
- [Roadmap](../roadmap) — **`stdout redirection`**, ordering of long-command work.
- [Exit codes & behavior](.) — non-interactive rules, **`generate`** catalog behaviour.
- [Verbosity & JSON](../cli/verbosity) — `--quiet`, `--silent`, `--json`
