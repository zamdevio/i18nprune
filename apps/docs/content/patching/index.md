...
Auto-patching keeps i18n loader wiring aligned after locale mutations (`generate`, `fill`, `sync`, `locales delete`) without requiring manual edits every run.
...
- **Shared CLI orchestration** — one patching handler path for `patch`, `sync --patch`, `fill --patch`, `generate --patch`, `locales delete --patch` (centralize envelope + `canAsk` / `--yes` / `--json`).
...
- **Tests** — resolver + loader variants; config injection status cases; optional integration `init → patch → sync --patch → fill`.
