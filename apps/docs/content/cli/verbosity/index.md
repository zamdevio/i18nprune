...
## `-v` / `--version` vs `version` subcommand

**`-v` / `--version`** are rewritten in **`preprocessArgv`** (`packages/cli/src/argv/index.ts`) to the **`version`** subcommand when you did not pass another command, so **`preAction`**, **`RunOptions`**, and the banner behave like **`i18nprune version`** (e.g. **`-q`** shows the box; **`-s`** prints only the semver line).
