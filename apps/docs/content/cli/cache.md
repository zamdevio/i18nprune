...
- **Benchmarking** apples-to-apples without cache masking wall time ([Performance hub](../performance#cli-project-cache)).
...
Automatic **`latest`** semver checks (**`ensureUpdateCacheRefreshed`**) use **`~/.config/i18nprune/updatestate.json`** (XDG-aware), not **`~/.i18nprune/cache/`**. See **[Versioning and npm update discovery](../versioning#cache-directory-24h-throttle)** — that file gates **registry fetch throttle**, not project scan payloads.
