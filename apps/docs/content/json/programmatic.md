...
**Normative CLI output:** [JSON output (`--json`)](.) — envelope, supported commands, parsing rules, and `meta.apiVersion`.
...
Issue **`code`** strings: [issue codes](../issues). Constants **`ISSUE_*`** are exported from **`core`**.
...
Surface parity references: [`docs/exports/README.md`](../exports).
