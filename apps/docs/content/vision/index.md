...
- **Explicit over magic** — Config file + env + CLI overrides merge in a documented order, with predictable scanner/config behavior.
...
- [Roadmap](../roadmap) — direction and planned work at a high level.
- [Architecture](../architecture) — how layers fit together.
- [Commands](../commands) — one page per subcommand.
- [Exports](../exports) — **`i18nprune`** (CLI), **`i18nprune/core`**, **`@i18nprune/core`** (engine).
