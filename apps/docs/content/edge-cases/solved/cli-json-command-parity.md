...
1. **Source of truth:** `COMMANDS_WITH_JSON_OUTPUT` — keep it aligned with commands that implement a primary stdout envelope (see [JSON output](../../json)).
...
4. **Issues:** Use stable **`issues[]`** codes ([issue codes](../../issues)); **`enrichIssuesWithDocHrefs`** attaches **`docHref`** for **`i18nprune.*`** codes.
...
- [JSON output (`--json`)](../../json) — contract and command list  
- [Issue codes](../../issues) — stable **`issues[]`** registry  
- [Command behaviors](../../behavior/commands.md) — snapshot table  
- [Prompts & CLI boundaries](../../prompts) — why **`--json`** skips interactive prompts  
