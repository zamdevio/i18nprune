...
Madge answers: **“How are my TypeScript modules wired together?”** It does **not** replace a **bundler** or **Worker** graph audit. For **import surfaces**, host tiers, and **shipping** checks (bundle / `node:` boundaries), use **[Runtime (host adapters)](../runtime)**.
...
So: **graph hygiene** and **runtime / bundle hygiene** are **related** but **not identical**. After adapters land on the paths you ship to web/edge, add **CI** checks that fail on forbidden **`node:`** imports for those entry graphs — see **[docs/runtime/README.md](../runtime)**.
...
- [Architecture overview](../architecture) — layers and data flow.
- [Project tree](../architecture/tree) — where `packages/core` sits in the repo.
- [Runtime (host adapters)](../runtime) — adapters, tiers, bundle / Worker safety notes.
