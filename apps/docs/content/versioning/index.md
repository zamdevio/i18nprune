...
- **`-v` / `--version`** on the root program are rewritten to **`version`** when you did not pass another command, so **`-q` / `-s`** and the banner behave the same as **`i18nprune version`** (see [CLI verbosity](../cli/verbosity)).
...
- [Doctor command](../commands/doctor) (diagnostics; update hints are global, not doctor-only)
