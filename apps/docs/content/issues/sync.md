...
**Who:** **`sync`**, **`runSync`**.  
**What to do:** Treat it as a precedence notice: strip wins and leaves are written as plain strings (`legacy_string` mode). Remove one flag for clearer intent. See [Locales metadata mode](../locales/metadata).
