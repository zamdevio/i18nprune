...
Merged **after** defaults and the config **file**, **before** discovery and CLI flags (see [Configuration README](.#merge-order)). Values are also listed in the **`i18nprune config --json`** output under the env snapshot (these keys only).
...
| **`I18NPRUNE_NO_UPDATE_CHECK`** | When truthy (`1`, `true`, `yes`), skip **all** npm registry update discovery: no background refresh, no banner hint, no **`version --check`** fetch. **`CI=true`** also skips (see [Versioning](../versioning)). |
