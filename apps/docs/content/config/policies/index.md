...
| **`policies.parity`** | **`quality`**, **`fill`**, **review-style** checks (e.g. “English-identical leaves”) | Excludes paths or values **only from those heuristic checks**, not from scanning or sync |

So **`parity` is not a duplicate of `exclude`**: `exclude` limits **sources** scanned; **`parity`** only suppresses noisy **quality signals** where matching English is expected (brand strings, placeholders, experimental keys, etc.). **`preserve`** is about **merge behavior**, not ignoring files during scan.
