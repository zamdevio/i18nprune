 ...
 *
 * Renders a 1200×630 SVG share card with the i18nprune branding and
 * live GitHub star count from the meta worker (GitHub + npm cache).
 *
 * Route: GET /og.svg
 * Cache: 5 min at edge, 1h browser. Live stars are best-effort.
 */
...
type OgMetaResult = { starsState: OgStarsState; cliVersion: string };

async function fetchOgMeta(): Promise<OgMetaResult> {
...
function renderSvg(state: OgStarsState, cliVersion: string): string {
  const starsLabel =
    state.kind === 'live'
...
export const onRequestGet: PagesFunction = async () => {
  ...
  const svg = renderSvg(starsState, cliVersion);
