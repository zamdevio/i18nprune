...
/** Stable JSON text for byte comparison against committed missing snapshots. */
export function normalizeMissingJsonEnvelope(stdout: string, fixtureAbs: string): string {
 ...
 */
export function normalizeMissingHumanStderr(raw: string, fixtureAbs: string): string {
  ...
  out = rest;
  out = out.replace(/missing · ok · \d+ms/g, 'missing · ok · <ms>');
  return `${out}\n`;
}
