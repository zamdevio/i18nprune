...
/** Stable JSON text for byte comparison against committed quality snapshots. */
export function normalizeQualityJsonEnvelope(stdout: string, fixtureAbs: string): string {
 ...
 */
export function normalizeQualityHumanStderr(raw: string, fixtureAbs: string): string {
  ...
  out = rest;
  out = out.replace(/quality · ok · \d+ms/g, 'quality · ok · <ms>');
  return `${out}\n`;
}
