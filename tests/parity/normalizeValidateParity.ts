...
/** Stable JSON text for byte comparison against committed validate snapshots. */
export function normalizeValidateJsonEnvelope(stdout: string, fixtureAbs: string): string {
 ...
 */
export function normalizeValidateHumanStderr(raw: string, fixtureAbs: string): string {
  ...
  out = rest;
  out = out.replace(/validate · ok · \d+ms/g, 'validate · ok · <ms>');
  return `${out}\n`;
}
