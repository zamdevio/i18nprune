...
/** Stable JSON text for byte comparison against committed sync snapshots. */
export function normalizeSyncJsonEnvelope(stdout: string, fixtureAbs: string): string {
 ...
 */
export function normalizeSyncHumanStderr(raw: string, fixtureAbs: string): string {
  ...
  out = rest;
  out = out.replace(/sync · ok · \d+ms/g, 'sync · ok · <ms>');
  return `${out}\n`;
}
